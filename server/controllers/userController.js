...
const login = async (req, res) => {
  try {
    ...
    const user = await User.findOne({ email }); // anajana , email,.....
    if (!user) {
      return res.status(404).json({
        status: 'failed',
...
const getAllUsers = async (req, res) => {
  try {
    const users = await User.find().populate('branchDetails');
...
const getMyUsers = async (req, res) => {
  try {
    ...
    const users =
      role === 'Admin'
        ? await User.find({ creatorId: userId }).populate('branchDetails')
        : await User.find({ branchDetails: branchId }).populate(
            'branchDetails'
          );
